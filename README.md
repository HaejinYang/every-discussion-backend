...
- Laravel, MySQL: Laravel을 선택한 이유는 APM 기술 스택에서 더 나아지고 싶었기 때문이다. 기존의 APM은 웹 서비스 개발을 간단하게 해주는 장점을 갖췄지만, 개발자마다 서로 다른 개발
  방식과 컨벤션으로 유지보수가 어려운 단점이 있다. Laravel을 통해 프레임워크 기반 개발이 주는 통일성 있고 효율적인 개발을 경험할 수 있었다. 또한, PHP의 패키지 매니저인 composer를 통하여
  안정적으로 라이브러리 관리할 수 있었다.
- Redis: Laravel이 제공하는 큐 시스템과 대시보드(horizon)를 활요하기 위하여 선택했다.
- AWS Lightsail: Lightsail을 선택한 이유는 웹 개발에 필요한 서비스를 한데 모아 제공하는 편리함과 상당한 수준의 기본 트래픽을 제공해 주었기 때문이다.
- Docker: Github Action과 Docker를 기반으로 배포 자동화를 구축했다. 개발자가 하나씩 하는 것은 실수를 유발하기 쉽기 때문이다. Docker는 개발 환경 구축이 쉽고 그대로 배포에도
  활용할 수 있어서 선택했다.
...
의견 등록, 수정 요청이 오면, ChatGpt API를 이용하여 의견을 요약한다.
컨트롤러에서 바로 동기적으로 처리하면 응답이 너무 늦어지므로, job을 생성하여 queue에서 처리할 수 있도록 구현했다.
queue를 모니터링하기 위해서 라라벨이 제공하는 horizon(모니터링)을 사용하였다.
...
회원 가입, 비밀번호 찾기 등을 위해서 SMTP로 이메일 인증을 진행하였다. 라라벨은 메일 파사드를 내장하고 있으며, env파일의 설정을 조작하는 것으로 SMTP를 사용할 수 있었다.
